functions{
  ...
  real loglogistic_lcdf (real y, real alpha, real beta) {
    return log(loglogistic_cdf(y|alpha, beta));
  }
  ...
  real loglogistic_lccdf (real y, real alpha, real beta){
    return log1p(loglogistic_cdf(y|alpha, beta));
  }
  // 
  ...
  // }
  // 
  vector lik_bern(int[] cured, matrix z, vector betaC){
    vector[num_elements(cured)] lik_b;
    for(i in 1:num_elements(cured)){
      lik_b[i] = bernoulli_logit_glm_lpmf({cured[i]}|z[i], 0, betaC);
    }
    return lik_b;
  }
  
  real cen_loglogistic_lpdf( real t, int cured, int is_censored, real scale, real shape, row_vector z, vector betaC) {
    real log_lik_cen;
    real log_lik_uncen;
    real prop; 
    real prob = 0;
    
    real lik_b =  bernoulli_logit_glm_lpmf({cured}|z, 0, betaC); 
...
model{
  ...
  for (i in 1:N) {
    real log_lik_t = cen_loglogistic_lpdf(t[i]| cured[i], is_censored[i], muU[i], 1/sigma, z[i], betaC);
    target += log_lik_t;
  }
}
...
generated quantities{
  ...
  for(i in 1:N){
    log_lik[i] = cen_loglogistic_lpdf(t[i]| cured[i], is_censored[i], muU[i], 1/sigma, z[i], betaC);
  }
