functions{
  ...
  real cen_loglogistic_lpdf( real t, int cured, int is_censored, real scale, real shape, row_vector z, vector betaC,  real delta, real lambda) {
    real log_lik_cen;
    real log_lik_uncen;
    real prop; 
    real prob = 0;
...
transformed parameters{
  ...
  // for (i in 1:N){
  //   log_lik[i] = cen_loglogistic_lpdf(t[i]| cured[i], is_censored[i], 1/sigma, muU[i], z[i], betaC, delta, lambda);
  // }
...
model{
  ...
  for (i in 1:N) {
    real log_lik_t = cen_loglogistic_lpdf(t[i]| cured[i], is_censored[i], muU[i], 1/sigma, z[i], betaC, delta, lambda);
    target += log_lik_t;
  }
...
generated quantities{
  ...
  for (i in 1:N) {
    log_lik[i] = cen_loglogistic_lpdf(t[i]| cured[i], is_censored[i], muU[i], 1/sigma, z[i], betaC, delta, lambda);
  }
}
