functions{
  ...
  real lik_bern_lpmf(int cured, row_vector z, vector betaC, real delta){
    ...
    real cen_weibull_lpdf( real t, int cured, int is_censored, real shape, real scale, row_vector z, vector betaC, real delta) {
      real log_lik_cen;
      real log_lik_uncen;
      real prop; 
      real prob = 0;
...
model{
  ...
  for (i in 1:N) {
    real log_lik_t = cen_weibull_lpdf(t[i]| cured[i], is_censored[i], 1/sigma, muU[i], z[i], betaC, delta);
    target += log_lik_t;
  }
...
generated quantities{
  ...
  for (i in 1:N) {
    curedd = cured[i]/1.0; 
    log_lik[i] = cen_weibull_lpdf(t[i]| cured[i], is_censored[i], 1/sigma, muU[i], z[i], betaC, delta);
  }
