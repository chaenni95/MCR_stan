functions{
  ...
  real lik_bern_lpmf(int cured, row_vector z, vector betaC, real lambda, real delta){
   ...
   real cen_lognormal_lpdf(real t, int cured, real is_censored, real muU, real sigma, row_vector z, vector betaC, real delta, real lambda) {
    real log_lik_cen;
    real log_lik_uncen;
    real prop; 
    real prob = 0;
...
model{
  ...
  for (i in 1:N){
    // real log_lik_i = lik_bern_lpmf(cured[i]| z[i], betaC, lambda, delta);
    real log_lik_t = cen_lognormal_lpdf(t[i]| cured[i], is_censored[i], muU[i], sigma, z[i], betaC, delta, lambda);
    target += log_lik_t;
  }
}
...
generated quantities{
  ...
  for (i in 1:N) {
    curedd = cured[i]/1.0; 
    log_lik[i] = cen_lognormal_lpdf(t[i]| cured[i], is_censored[i], muU[i], sigma, z[i], betaC, delta, lambda);
    }
}
