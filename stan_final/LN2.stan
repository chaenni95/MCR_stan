functions{
   ...
   real cen_lognormal_lpdf(real t, real is_censored, real muU, real sigma, row_vector z, vector betaC, real alpha_s) {
    real log_lik_cen;
    real log_lik_uncen;
    real prop; 
    real prob = 0;
...
model{
  ...
  for (i in 1:N){
    real log_lik_t = cen_lognormal_lpdf(t[i]| is_censored[i], muU[i], sigma, z[i], betaC, alpha_s);
    target += log_lik_t;
  }
}
...
generated quantities{
  vector[N] log_lik; 
  for (i in 1:N) {
    log_lik[i] = cen_lognormal_lpdf(t[i]|is_censored[i], muU[i], sigma, z[i], betaC, alpha_s);
  }
}
