functions{
  ...
  real cen_weibull_lpdf(real t, int is_censored, real shape, real scale, row_vector z, vector betaC) {
...
model{
  ...
  for (i in 1:N) {
    real log_lik_t = cen_weibull_lpdf(t[i]|is_censored[i], 1/sigma, muU[i], z[i], betaC);
    target += cen_weibull_lpdf(t[i]|is_censored[i], 1/sigma, muU[i], z[i], betaC);
  }
...
generated quantities{
  ...
  for(i in 1:N){
    log_lik[i] = cen_weibull_lpdf(t[i]|is_censored[i], 1/sigma, muU[i], z[i], betaC);
  }
}
