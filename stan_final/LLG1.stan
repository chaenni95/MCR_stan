functions{
  ...
  real cen_loglogistic_lpdf( real t, int is_censored, real scale, real shape, row_vector z, vector betaC) {
    real log_lik_cen;
    real log_lik_uncen;
    real prop; 
    real prob = 0;
...
model{
  ...
  for (i in 1:N) {
    real log_lik_t = cen_loglogistic_lpdf(t[i]|is_censored[i], muU[i], 1/sigma, z[i], betaC);
    target += log_lik_t;
  }
}
...
generated quantities{
  ...
  vector[N] log_lik; 
  for(i in 1:N){
    log_lik[i] = cen_loglogistic_lpdf(t[i]|is_censored[i], muU[i], 1/sigma, z[i], betaC);
  }
