...
### generating Mixture Cure Model with Weibull base

geraMCM.WB <- function(n, x, w, censoring_time=10 , beta=c(1,2), eta=c(1,2), alpha=1.5, link="fglogit", tau=1.5, delta = 2){
...
stanmodel <- stan_model("/home/chy20005/Recidivism/simulation/wb4.stan")

for (i in 1:num_reps) {
  
  # Step 1: Generate a new dataset (modify this function to fit your simulation setup)
  data <- geraMCM.WB(n, x, w, censoring_time=censoring_time , beta=beta, eta=eta, alpha=alpha,link="fglogit", tau=1.5, delta = 2)  # Custom function to simulate data
  ...
  fit <- sampling(stanmodel, data = stan_data, seed = 12345, chains = 1, iter = 1000, 
                  init = 0, pars = c("betaU", "betaC", "sigma", "lambda", "delta", "lp__", "log_lik"))
...
saveRDS(results, "/home/chy20005/Recidivism/simulation/res2/simwb4.rds")
...
saveRDS(summarize_simulations(simulation_summaries), "/home/chy20005/Recidivism/simulation/res2/simwb4_summary.rds")
